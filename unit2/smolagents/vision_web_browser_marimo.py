...
def _(mo):
    import argparse
    class MarimoArgumentParser(argparse.ArgumentParser):
        def parse_args(self, args=None, namespace=None):
            if args is None:
                args = mo.cli_args()  # Use mo.args instead of sys.argv
            return super().parse_args(args, namespace)
    return MarimoArgumentParser, argparse


@app.cell
def _(MarimoArgumentParser):
    ...
    def parse_arguments():
        parser = MarimoArgumentParser(description="Run a web browser automation script with a specified model.")
        parser.add_argument(
            "prompt",
            type=str,
    ...
    def run():
        ...
        result = agent.run(args.prompt + helium_instructions)
        return result
...
def _(mo, run):
    ...
    if isinstance(_res, str):
        mo.output.replace(mo.md(_res))
    else:
        mo.output.replace(_res)
    return
